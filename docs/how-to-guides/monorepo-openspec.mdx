---
title: Monorepo OpenSpec
slug: /monorepo-openspec
description: How to run OpenSpec in a monorepo with package-local specs and centralized change tracking.
---

import { SectionHeader, CalloutBox } from '@site/src/components/tom';

<SectionHeader
  num="10"
  title="Monorepo OpenSpec"
  desc="Specs live with packages. OpenSpec tracks changes centrally. Multi-path discovery keeps teams unblocked."
/>

<CalloutBox type="teal" label="Diataxis">
  How-to. This page shows a practical monorepo setup. If you want to understand the discovery algorithm first, see [/workflow-diagrams](/workflow-diagrams).
</CalloutBox>

## TL;DR

1. One root `agnostic-agent.yaml` enables multi-path spec discovery.
2. Specs live near code: `packages/*/specs/` (and optionally `apps/*/specs/`, `specs/`).
3. OpenSpec changes track centrally: `.agentic/openspec/changes/`.
4. CLI auto-discovers all specs across packages.
5. Teams implement in parallel per package.
6. Prompts help with integration and sequencing: [/prompting-guide](/prompting-guide).

**Core boundary rule:** Component repos (individual packages) use OpenSpec ONLY. Platform-level tooling (BMAD, Speckit) stays in the platform repo. Use `platform-ref.yaml` in component repos to reference platform alignment without importing platform tools.

## One-Time Setup

### 1) Root config

Create or update `agnostic-agent.yaml` in repo root:

```yaml title="agnostic-agent.yaml"
project:
  name: MyMonorepo
  version: 0.1.0
  roots:
    - .
agents:
  defaults:
    max_tokens: 4000
    model: claude-3-5-sonnet-20241022
paths:
  openSpecDir: .agentic/openspec/changes
  specDirs:
    - packages/*/specs
    - apps/*/specs
    - specs
  contextDirs:
    - .agentic/context
```

<details>
  <summary>Diagram: Multi-path spec discovery (first match wins)</summary>

```text
┌──────────────────────────────────────────────────────────────────┐
│         MONOREPO: Spec Discovery & Resolution                   │
└──────────────────────────────────────────────────────────────────┘

agnostic-agent.yaml (root config):
┌────────────────────────────────────┐
│ paths:                             │
│   specDirs:                        │
│     - packages/*/specs     ← Rule 1│
│     - apps/*/specs         ← Rule 2│
│     - specs                ← Rule 3│
└────────────────────────────────────┘

When you run:
  agentic-agent openspec init "Name" --from auth-oauth.md

CLI searches in order (FIRST MATCH WINS):
┌──────────────────────────────────────────────────────────────────┐
│ 1. packages/*/specs/auth-oauth.md                               │
│    ├─ packages/auth-service/specs/auth-oauth.md   ← FOUND! ✓   │
│    ├─ packages/web-ui/specs/auth-oauth.md         (skip)       │
│    └─ packages/api/specs/auth-oauth.md            (skip)       │
│                                                                  │
│ 2. apps/*/specs/auth-oauth.md        (skipped, rule 1 matched) │
│                                                                  │
│ 3. specs/auth-oauth.md               (skipped, rule 1 matched) │
└──────────────────────────────────────────────────────────────────┘

Result:
  ✓ Found: packages/auth-service/specs/auth-oauth.md
  ✓ Creates: .agentic/openspec/changes/auth-oauth/
  ✓ Tracks centrally, specs live with packages
```

</details>

### 2) Package spec directories

```bash
mkdir -p packages/service-a/specs
mkdir -p packages/service-b/specs
mkdir -p specs
```

## Create Specs Per Package

### Recommended structure

```text
packages/
├── service-a/
│   ├── specs/
│   │   ├── auth-oauth.md
│   │   └── logging-upgrade.md
│   └── src/
└── service-b/
    ├── specs/
    │   └── webhook-retry.md
    └── src/
```

### Example spec file

`packages/service-a/specs/auth-oauth.md`

```markdown
# OAuth Integration

## Problem
Users can only authenticate with username/password. We need OAuth support (Google, GitHub).

## Solution
Integrate OAuth 2.0 flow using industry-standard libraries.

## What Changes
- Add OAuth provider abstraction
- Create /auth/callback endpoint
- Store provider tokens in database
- Update login UI with provider buttons

## Acceptance Criteria
- [ ] Users can login with Google
- [ ] Users can login with GitHub
- [ ] Tokens refresh automatically
- [ ] Existing password auth still works
- [ ] Tests cover both flows

## Notes
- Avoid vendor lock-in to single OAuth library
- Keep token storage encrypted
- Set refresh token expiry to 30 days
```

## Initialize OpenSpec From a Package Spec

```bash
agentic-agent openspec init "OAuth Integration" \
  --from packages/service-a/specs/auth-oauth.md
```

What happens:
- CLI finds the spec via `specDirs` (first match wins).
- It creates a change under `.agentic/openspec/changes/<change-id>/`.
- It generates (or updates) `proposal.md` and `tasks.md`.

## Work Task-by-Task (Per Package)

```bash
agentic-agent task list
agentic-agent task claim <task-id>

cd packages/service-a
# ... implement + tests ...
cd ../..

agentic-agent task complete <task-id>
```

## Parallel Work Pattern

The monorepo benefit is parallelism:

1. Create one spec per package (or per change) near the code.
2. Initialize OpenSpec changes for each package spec.
3. Each team works inside its package, claiming tasks.
4. Integrate at the end (contracts, rollout order, integration tests).

For dependency/rollout sequencing, see:
- [/workflow-diagrams](/workflow-diagrams) (dependency graph)
- [/prompting-guide](/prompting-guide) (dependency analysis prompt)

<details>
  <summary>Diagram: Monorepo workflow (multiple packages in parallel)</summary>

```text
┌──────────────────────────────────────────────────────────────┐
│       MONOREPO: Multi-Package Parallel Workflow              │
└──────────────────────────────────────────────────────────────┘

monorepo/
├── agnostic-agent.yaml      ← Root config (multi-path specDirs)
├── specs/                   ← Shared specs
├── packages/
│   ├── service-a/
│   │   ├── specs/
│   │   │   ├── auth-oauth.md        ← Package-specific spec
│   │   │   └── logging-upgrade.md
│   │   └── src/
│   ├── service-b/
│   │   ├── specs/
│   │   │   ├── webhook-retry.md
│   │   │   └── db-migration.md
│   │   └── src/
│   └── service-c/
│       ├── specs/
│       │   └── api-v2.md
│       └── src/
└── .agentic/
    └── openspec/changes/    ← Centralized tracking
        ├── auth-oauth/
        ├── webhook-retry/
        └── api-v2/

WORKFLOW (PARALLEL):

  Day 1: PLAN ALL FEATURES
  ┌──────────────────────────────────────┐
  │ agentic-agent openspec init "Auth"   │
  │   --from packages/service-a/specs/.. │
  │                                      │
  │ agentic-agent openspec init "Webhook"│
  │   --from packages/service-b/specs/.. │
  │                                      │
  │ agentic-agent openspec init "API"    │
  │   --from packages/service-c/specs/.. │
  └──────────────────────────────────────┘
            ↓
  Days 2-5: BUILD IN PARALLEL
  ┌────────────────────────────────────────────────────┐
  │ Team A (service-a):                               │
  │   cd packages/service-a                           │
  │   agentic-agent task claim <task-1>              │
  │   [code] → task complete                          │
  │                                                   │
  │ Team B (service-b):                    PARALLEL   │
  │   cd packages/service-b                           │
  │   agentic-agent task claim <task-2>              │
  │   [code] → task complete                          │
  │                                                   │
  │ Team C (service-c):                               │
  │   cd packages/service-c                           │
  │   agentic-agent task claim <task-3>              │
  │   [code] → task complete                          │
  └────────────────────────────────────────────────────┘
            ↓
  Day 6: INTEGRATE & TEST
  ┌──────────────────────────────────────┐
  │ Service A provides OAuth tokens      │
  │   ↓                                  │
  │ Service B & C consume tokens         │
  │   ↓                                  │
  │ Run integration tests                │
  └──────────────────────────────────────┘
            ↓
  Day 7: VERIFY & RELEASE
  ┌──────────────────────────────────────┐
  │ agentic-agent openspec complete auth │
  │ agentic-agent openspec complete ..   │
  │ agentic-agent openspec archive auth  │
  └──────────────────────────────────────┘
            ↓
        ✅ ALL DONE
```

</details>

<details>
  <summary>Diagram: Multi-service dependency graph (implementation order)</summary>

```text
┌──────────────────────────────────────────────────────────────┐
│       EXAMPLE: OAuth Feature Across 3 Services              │
└──────────────────────────────────────────────────────────────┘

INITIAL SPECS (in each package):
┌──────────────┐      ┌──────────────┐      ┌──────────────┐
│   Auth Svc   │      │   API Svc    │      │   Web UI     │
│              │      │              │      │              │
│ OAuth        │      │ Token        │      │ Login Form   │
│ Provider     │      │ Verification │      │ OAuth Buttons│
│              │      │              │      │              │
│ Provides:    │      │ Needs:       │      │ Needs:       │
│ - POST /auth │      │ - OAuth      │      │ - OAuth      │
│ - tokens     │      │   tokens     │      │   endpoints  │
└──────────────┘      └──────────────┘      └──────────────┘

DEPENDENCIES (Prompt #6 analysis):
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  Auth Svc (START HERE)                                      │
│  ├─ Implement OAuth endpoints                               │
│  ├─ Add token generation                                    │
│  ├─ Tests pass ✓                                            │
│  └─ Deploy to staging                                       │
│           ↓                                                  │
│  API Svc (WAIT FOR AUTH)                                    │
│  ├─ Add token verification middleware                       │
│  ├─ Call Auth Svc /verify endpoint                         │
│  ├─ Tests pass ✓                                            │
│  └─ Deploy to staging                                       │
│           ↓                                                  │
│  Web UI (LAST)                                              │
│  ├─ Add login form with OAuth buttons                       │
│  ├─ Call Auth Svc endpoints                                │
│  ├─ Store tokens in localStorage                           │
│  ├─ Tests pass ✓                                            │
│  └─ Deploy                                                  │
│                                                              │
└──────────────────────────────────────────────────────────────┘

BLOCKERS & RISKS:
┌──────────────────────────────────────────────────────────────┐
│ ❌ If Auth Svc not ready                                    │
│    → API Svc can mock /verify endpoint                      │
│                                                              │
│ ❌ If API Svc not ready                                     │
│    → Web UI can call Auth Svc directly (temporary)          │
│                                                              │
│ ✅ Contract first                                           │
│    → Agree API contracts before implementation              │
│    → Mock endpoints until real service ready                │
└──────────────────────────────────────────────────────────────┘
```

</details>

## Workflow: From Spec to Production

### Day 1: Define spec

```bash
cat > packages/service-a/specs/auth-oauth.md << 'EOF'
# OAuth Integration
...
EOF

agentic-agent openspec init "OAuth Integration" \
  --from packages/service-a/specs/auth-oauth.md
```

Use Prompt #1 (Spec Review) to decompose into tasks.

### Days 2-5: Implement

```bash
agentic-agent task claim <task-id>
cd packages/service-a
# ... edit src/auth/*.ts, add tests ...
npm test
cd ../..
agentic-agent task complete <task-id>
```

Use Prompt #2 (Package-Specific) to guide each implementation.

### Day 6: Integrate (if multi-package)

```bash
agentic-agent openspec list
cd packages/service-b
# ... implement integration ...
# Verify integration with Prompt #3 (Integration)
```

### Day 7: Verify and complete

```bash
agentic-agent openspec show oauth-integration
# Use Prompt #5 (Verification) to ensure all criteria met
agentic-agent openspec complete oauth-integration
agentic-agent openspec archive oauth-integration
```

## Multi-Service Example: Payment Flow

```text
packages/
├── billing-service/
│   └── specs/
│       └── subscription-billing.md
├── payments-service/
│   └── specs/
│       └── payment-processing.md
└── customer-api/
    └── specs/
        └── billing-endpoints.md
```

```bash
# Day 1: Define specs for all three services
agentic-agent openspec init "Subscription Billing" \
  --from packages/billing-service/specs/subscription-billing.md

agentic-agent openspec init "Payment Processing" \
  --from packages/payments-service/specs/payment-processing.md

agentic-agent openspec init "Billing API Endpoints" \
  --from packages/customer-api/specs/billing-endpoints.md

# Days 2-5: Implement in parallel (one team per service)

# Day 6: Integration testing
# Use Prompt #3 (Integration) to verify contracts between services

# Day 7: Verify all three
agentic-agent openspec complete subscription-billing
agentic-agent openspec complete payment-processing
agentic-agent openspec complete billing-endpoints
```

## CLI Reference (Common)

| Command | Purpose |
|---|---|
| `openspec init <name> --from <file>` | create/update a change from a spec |
| `openspec list` | list all changes |
| `openspec show <id>` | show a change (includes original spec path) |
| `openspec status <id>` | show progress |
| `openspec complete <id>` | mark change done |
| `openspec archive <id>` | archive change |
| `task create` / `task claim` / `task complete` | task loop |

## FAQ

### Can I have root-level specs too?

Yes. Include `specs/` in `specDirs` for shared/cross-package specs.

### What if two packages have specs with the same name?

The first match wins (based on `specDirs` ordering). Avoid collisions by namespacing or unique filenames.

### Do I need one OpenSpec change per spec file?

No. Create OpenSpec changes when you want lifecycle tracking. For tiny work, use tasks only.

### Can I organize specs differently?

Yes. Update `specDirs` in agnostic-agent.yaml:

```yaml
paths:
  specDirs:
    - docs/specs            # Centralized specs
    - packages/*/specs      # Package-level overrides
```

### How do I handle cross-package dependencies?

Use Prompt #6 (Dependency Analysis) from [/prompting-guide](/prompting-guide) to determine order. Then: Service A starts first (provider), Service B waits for A's API to stabilize, Service C integrates both.

### What if requirements change during implementation?

Update the spec file directly, then regenerate tasks:

```bash
vim packages/service-a/specs/auth-oauth.md
agentic-agent openspec init "OAuth Integration" \
  --from packages/service-a/specs/auth-oauth.md
```

## Next

- Single package workflow: [/small-projects](/small-projects)
- Commands: [/cli-cheatsheet](/cli-cheatsheet)
- Prompt library: [/prompting-guide](/prompting-guide)
