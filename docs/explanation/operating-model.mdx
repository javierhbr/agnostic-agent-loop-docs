...
slug: /explanation/operating-model
sidebar_label: Operating Model
description: The full Unified Spec-Driven Development operating model with 5-phase lifecycle, governance, agents, and scaling guide.
---
...
- Heavy methodologies (router services, gate validators, contract registries as servers) add infrastructure that nobody maintains

The answer is not more process. It is **governed, consumable knowledge**. A 5-phase model with change packages as the canonical execution unit, and zero overhead services -- gates live as checklists inside templates, not in TypeScript validators.

---

### 1.3 Methodology Origins -- What We Took and Why
...
**SDD -- Spec-Driven Development:** Spec-before-code rule, acceptance criteria in Given/When/Then, exit gates as self-verification checklists embedded in templates.

**MCP -- Model Context Protocol:** Platform MCP (one instance serving context packs, templates, agent definitions, workflow configs) and Component MCP (N instances, same image, serving contracts, invariants, patterns, and decisions per component).

**ADR -- Architecture Decision Records:** Decisions as first-class Markdown artifacts surfaced via `get_decisions()`. Open ADRs block the Verifier from clearing the merge gate.

**Agile / XP:** Risk-proportional process (quick workflow for low-risk, full workflow for critical). Small deliverables per agent -- each produces one or two files and hands off.

**Lean:** The golden rule -- eliminate what doesn't change output. Logic in files, not infrastructure. Knowledge feedback loop -- every implementation cycle updates component MCP sources.
...
|   Platform MCP . Component MCPs (xN)                        |
...
### 4.5 Two MCPs and Four Agents

**Platform MCP -- 1 instance per organisation**

Reads the specs repo. Never requires redeployment -- update the specs repo and the MCP serves new knowledge immediately.
...
One instance per component. Serves that component's contracts, invariants, patterns, and technical decisions.
...
**Four Agents -- Markdown files in `.claude/agents/`**
...
The workflow activated depends on risk level:
...
| **Full** | high / critical | Analyst --> Architect --> Developer --> Verifier + Human Gate |
...
```
  Risk Level?  (Platform MCP decides)
       ...
       |
       +--- medium ----------------> STANDARD: Architect --> Developer (parallel) --> Verifier
       |
       +--- high / critical -------> FULL: Analyst --> Architect --> Developer (parallel) --> Verifier + Human Gate
```

Developers fan out **in parallel per component** once the feature-spec is ready.
...
## 12) Two-Iteration Adoption Plan
...
| Rework after costly integration bugs | Contracts versioned, gate-checked before code |
...
- [ ] Analyst activated for high-risk features
